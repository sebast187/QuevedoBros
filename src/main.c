...
// ==========================================
#define TILE_SIZE 40                 // Every block in the game is 40x40 pixels
#define GRAVITY 1800.0f              // Downward pull (positive Y goes DOWN in Raylib)
#define JUMP_SPEED -720.0f           // Initial jump burst (negative Y goes UP)

// --- PHYSICS CONSTANTS (Mario Style Momentum) ---
#define PLAYER_MAX_SPEED 350.0f      // Top running speed
#define PLAYER_ACCEL 2000.0f         // How quickly the player reaches max speed
#define PLAYER_FRICTION 1400.0f      // How fast the player slides to a stop on the ground
#define PLAYER_AIR_DRAG 300.0f       // Air resistance (gives less control in the air than on ground)
#define SKID_THRESHOLD 100.0f        // Speed required to trigger the skidding particle effect
...
// Tracks what screen the game is currently showing
typedef enum { 
...
// Tracks the visual style of the current level to change colors/music
typedef enum { THEME_GRASS, THEME_CAVE, THEME_SNOW, THEME_CASTLE } LevelTheme;

// Defines how an enemy behaves
typedef enum { 
    ENEMY_WADDLER,  // Walks off cliffs (Goomba)
    ENEMY_HOPPER,   // Jumps and turns around at cliffs (Green Frog)
    ENEMY_DROPPER   // Waits in the sky and drops when player is under (Thwomp)
} EnemyType;

typedef struct {
    Vector2 pos;        // X, Y position
    Vector2 vel;        // X, Y velocity (movement speed/direction)
    int width, height;  // Size of the physical collision box
    bool isGrounded;    // True if standing on a solid object
    bool isDead;        
    bool facingRight;   // Used to flip the procedural graphics
    int score;
    int lives;
} Player;
...
typedef struct {
    Vector2 pos;
    Vector2 vel;
    bool active;        // If false, the engine ignores it (dead or unspawned)
    int width, height;
    float timer;        // Used for jump timing and animation wobbling
    EnemyType type;
    bool isGrounded;
} Enemy;
...
typedef struct {
    ...
    bool active;
    float size;
    float rotation;     // Used for spinning brick debris
} Particle;
...
// GLOBAL VARIABLES
// ==========================================
char current_map[MAP_HEIGHT][MAP_WIDTH];  // The active 2D grid loaded from levels.h
Player player;
...
Camera2D camera = { 0 };                  // The virtual camera that follows the player
float targetCamY = 0.0f;                  // Used for smooth vertical camera panning (Lerp)
LevelTheme currentTheme = THEME_GRASS;

// --- AUDIO GLOBALS ---
Music themeMusic[4];         // Array holding all 4 background songs
Music* currentMusic = NULL;  // Pointer tracking which song is currently playing
Sound sfxJump;
Sound sfxCoin;
Sound sfxBreak;
Sound sfxDeath;
...
// --- FLAGPOLE GLOBALS ---
Vector2 flagPos = {0};       // Where the flagpole is located on the map
bool hasFlag = false;        // Does this level have a flagpole?
...
int player_scores[2];
int player_levels[2];
int player_return_levels[2] = {-1, -1}; // Remembers where to return after beating a Secret Level

// ==========================================
// SYSTEM & HELPER FUNCTIONS
// ==========================================

// macOS sets the Working Directory to the root of the hard drive when opening an .app Bundle.
// This function forces the game to look for "resources/" in the correct location.
void SetupMacPaths() {
    if (FileExists("resources/bgm_grass.mp3")) return; // Terminal execution is fine
    ChangeDirectory(GetApplicationDirectory());
    if (DirectoryExists("../Resources")) ChangeDirectory("../Resources"); // Inside .app Bundle
    else if (DirectoryExists("../resources")) ChangeDirectory("../");     // Fallback
}
...
float SmoothCam(float start, float end, float amount) { return start + amount * (end - start); }

// Moves a value towards a target by a fixed amount (used for friction sliding)
float MoveTowards(float current, float target, float maxDelta) {
    if (fabs(target - current) <= maxDelta) return target;
    return current + (target > current ? maxDelta : -maxDelta);
}

// Determines the visual theme based on the level index
LevelTheme GetLevelTheme(int level) {
    if (level == 8) return THEME_CAVE;    // Secret 1
    if (level == 9) return THEME_SNOW;    // Secret 2
    if (level == 10) return THEME_CASTLE; // Secret 3
...
void SaveGame() {
    ...
    if (file) {
        ...
        fclose(file);
        saveMessageTimer = 2.0f; // Shows "GAME SAVED" popup for 2 seconds
    }
}
...
// Emits 4 spinning brick chunks when a 'B' block is hit
void SpawnDebris(Vector2 pos) {
    for (int i = 0; i < 4; i++) SpawnParticle(pos, (Color){139, 69, 19, 255}, 1.5f, 12);
}

// Reads the strings in levels.h and converts them into the live 2D array, 
// placing the player and spawning enemies where specified.
void LoadLevel(int level_idx) {    
    ...
    // If the theme changes, stop the old track and start the new one!
    ...
    // Wipe the old map clean
    for (int y = 0; y < MAP_HEIGHT; y++) {
        for (int x = 0; x < MAP_WIDTH; x++) current_map[y][x] = ' ';
    }

    // Load the new map layer by layer
    for (int y = 0; y < MAP_HEIGHT; y++) {
        ...
        for (int x = 0; x < MAP_WIDTH; x++) {
            ...
            if (tile == 'P') { // Player Spawn Point
                player.pos = (Vector2){ x * TILE_SIZE, y * TILE_SIZE };
                player.vel = (Vector2){ 0, 0 };
                player.isDead = false;
                player.facingRight = true;
                current_map[y][x] = ' '; // Erase the P so it doesn't draw as a block
                
                // Snap camera to player immediately so it doesn't pan across the map
                targetCamY = player.pos.y - 100;
                camera.target = (Vector2){ player.pos.x, targetCamY };
            }
            else if (tile == 'E' || tile == 'H' || tile == 'D') { // Enemy Spawn Points
            ...
            else if (tile == 'F') { // Flagpole location
                flagPos = (Vector2){ x * TILE_SIZE, y * TILE_SIZE };
                hasFlag = true;
                current_map[y][x] = ' '; // Hide raw F from standard collision logic
            }
        }
    }
    // Deactivate leftover enemies and particles from the previous level
    for (int i = enemy_count; i < MAX_ENEMIES; i++) enemies[i].active = false;
    for (int i = 0; i < MAX_PARTICLES; i++) particles[i].active = false;
}

// Determines if a tile stops the player/enemies from moving through it
...
void UpdatePhysics(float dt) {
    ...
    // VARIABLE JUMP: If the player releases the jump key while going up, apply heavy gravity to pull them down early!
    ...
    // USE FULL HITBOX: No shrinking necessary!
    int leftTileX = player.pos.x / TILE_SIZE;
    int rightTileX = (player.pos.x + player.width - 0.01f) / TILE_SIZE;
    int topTileX = player.pos.y / TILE_SIZE; 
    int bottomTileX = (player.pos.y + player.height - 0.01f) / TILE_SIZE; 
        ...
        for (int x = leftTileX; x <= rightTileX; x++) {
            if (x >= 0 && x < MAP_WIDTH && y >= 0 && y < MAP_HEIGHT) {
                ...
                if (IsTileSolid(t)) {
                    ...
                    // 1. TOE STUB FIX: If falling and clipping the top 12 pixels of a block, ignore X collision!
                    // This lets the player slide smoothly onto the block via the Y-Axis check.
                    if (player.vel.y >= 0 && playerBottom > blockTop && (playerBottom - blockTop) <= 12.0f) {
                        continue; 
                    }
                    
                    // 2. HEAD BONK FIX: If jumping up and clipping the bottom 12 pixels of a block, ignore X!
                    if (player.vel.y < 0 && playerTop < blockBottom && (blockBottom - playerTop) <= 12.0f) {
                        continue; 
                    }
    ...
    // "HOLE SNAGGING" FIX: Shrink X bounds by 2.5 pixels on each side.
    // This prevents the gravity check from getting caught on side-walls while falling down tight 1x1 shafts.
    int leftTileY = (player.pos.x + 2.5f) / TILE_SIZE;
    int rightTileY = (player.pos.x + player.width - 2.5f) / TILE_SIZE;
    ...
    for (int y = topTileY; y <= bottomTileY; y++) {
        for (int x = leftTileY; x <= rightTileY; x++) {
            if (x >= 0 && x < MAP_WIDTH && y >= 0 && y < MAP_HEIGHT) {
                ...
                if (IsTileSolid(t)) {
                    if (player.vel.y > 0) { // FALLING DOWN
                        player.pos.y = y * TILE_SIZE - player.height; // Snap to top of block
                        player.vel.y = 0;
                        player.isGrounded = true;
                    } else if (player.vel.y < 0) { // HITTING HEAD ON CEILING
                        player.pos.y = y * TILE_SIZE + TILE_SIZE; // Push down
                        player.vel.y = 0;
                        
                        if (t == '?') {
                            current_map[y][x] = 'X'; // Convert Mystery to Hard block
                            player.score += 100;
                            for (int i=0; i<8; i++) SpawnParticle((Vector2){x*TILE_SIZE+20, y*TILE_SIZE}, GOLD, 1.0f, 6);
                        }
                        else if (t == 'B') { 
                            current_map[y][x] = ' '; // Destroy Brick
                            player.score += 50;
                            PlaySound(sfxBreak);
    ...
    // Death by falling out of the world bounds
    if (player.pos.y > MAP_HEIGHT * TILE_SIZE + 200) player.isDead = true;
    
    // ------------------------------------------
    // 3. TALL FLAGPOLE COLLISION & SCORING
    // ------------------------------------------
    if (hasFlag) {
        Rectangle playerRec = { player.pos.x, player.pos.y, player.width, player.height };
        // Create an invisible, massive hitbox for the flagpole spanning 4 tiles high
        ...
        if (CheckCollisionRecs(playerRec, flagRec)) {
            ...
            // Calculate exactly where the player grabbed the pole (0.0 to 1.0)
            float percent = (poleBottom - hitY) / (poleBottom - poleTop);
            if (percent < 0.0f) percent = 0.0f;
            if (percent > 1.0f) percent = 1.0f;
            
            // Mario-style Tiered Points System based on height
            int points = 200;
            ...
            // Generate a Particle Explosion (Gold for top, Silver for bottom)
            ...
            hasFlag = false; // Prevent logic from firing multiple times
        }
    }
}
...
void UpdateEnemies(float dt) {
    for (int i = 0; i < MAX_ENEMIES; i++) {
        ...
        if (enemies[i].type == ENEMY_WADDLER) {
            ...
            // Drop instantly if Player walks exactly underneath them!
            if (enemies[i].vel.y == 0 && fabs(player.pos.x - enemies[i].pos.x) < 40 && player.pos.y > enemies[i].pos.y) {
                enemies[i].vel.y = 600.0f;
            } else if (enemies[i].vel.y != 0) {
                enemies[i].vel.y += GRAVITY * dt; // Continue falling once triggered
            }
        }
        ...
        int topTileX = enemies[i].pos.y / TILE_SIZE;
        int bottomTileX = (enemies[i].pos.y + enemies[i].height - 0.01f) / TILE_SIZE;
                ...
                if (x >= 0 && x < MAP_WIDTH && y >= 0 && y < MAP_HEIGHT) {
                    if (IsTileSolid(current_map[y][x])) {
                        if (enemies[i].type != ENEMY_DROPPER) {
                            ...
                            // Enemy Corner Forgiveness
                            float blockTop = y * TILE_SIZE;
                            float enemyBottom = enemies[i].pos.y + enemies[i].height;
                            if (enemies[i].vel.y >= 0 && enemyBottom > blockTop && (enemyBottom - blockTop) <= 10.0f) {
                                continue; 
                            }

                            if (enemies[i].vel.x > 0) { enemies[i].pos.x = x * TILE_SIZE - enemies[i].width; } 
                            else if (enemies[i].vel.x < 0) { enemies[i].pos.x = x * TILE_SIZE + TILE_SIZE; }
                            hitWall = true;
                        }
                    }
                }
        ...
        // --- SMART LEDGE CHECK (Hoppers Only) ---
        bool hitLedge = false;
        if (enemies[i].type == ENEMY_HOPPER && enemies[i].isGrounded) {
            ...
            if (pTX >= 0 && pTX < MAP_WIDTH && pTY < MAP_HEIGHT) {
                // If there is no solid block ahead of them, turn back!
                if (!IsTileSolid(current_map[pTY][pTX])) {
                    hitLedge = true;
                    // Snap safely back to the edge to prevent glitching down the wall
        ...
        // Apply Reversal if a wall or ledge was hit
        ...
        for (int y = topTileY; y <= bottomTileY; y++) {
            for (int x = leftTileY; x <= rightTileY; x++) {
                if (x >= 0 && x < MAP_WIDTH && y >= 0 && y < MAP_HEIGHT) {
                    if (IsTileSolid(current_map[y][x])) {
                        if (enemies[i].vel.y > 0) { // Landing
                            enemies[i].pos.y = y * TILE_SIZE - enemies[i].height;
                            enemies[i].vel.y = 0;
                            enemies[i].isGrounded = true; 
                            
                            // Droppers make a smash particle effect when they hit the floor
                            if (enemies[i].type == ENEMY_DROPPER) {
                                for (int p=0; p<10; p++) SpawnParticle(enemies[i].pos, GRAY, 1.0f, 4);
                            }
                        }
                    } else if (current_map[y][x] == '^') {
                        // Enemies instantly burst into a puff of blood/smoke if they fall on spikes
        ...
        if (CheckCollisionRecs(pRec, eRec)) {
            // Check if player is falling AND their feet are above the enemy's vertical center (Head Stomp!)
            // Droppers cannot be stomped!
            if (enemies[i].type != ENEMY_DROPPER && player.vel.y > 0 && player.pos.y + player.height < enemies[i].pos.y + 20) {
                enemies[i].active = false;
                player.vel.y = JUMP_SPEED * 0.7f; // Bounce off the enemy
                player.score += (enemies[i].type == ENEMY_HOPPER) ? 400 : 200; // Frogs are worth more
        ...
        // Remove enemy from memory if they fall far below the camera
...
// ==========================================
void DrawGame() {
    // 1. DYNAMIC PARALLAX BACKGROUNDS (Color gradients based on the Theme Enum)
    Color skyTop, skyBot;
    ...
    // Draw procedurally generated background elements (Clouds, Stalactites, Snow, Lava Ash)
    for(int i = 0; i < 40; i++) {
        // Panning math to make backgrounds scroll slower than the foreground (Parallax effect)
    ...
    BeginMode2D(camera); // Begin drawing relative to the player's camera!
    
    // Draw the raw Flagpole graphics manually, since its map tile was hidden for custom collision
    if (hasFlag) {
        DrawRectangle(flagPos.x + 16, flagPos.y - 120, 6, 160, LIGHTGRAY);
        DrawCircle(flagPos.x + 19, flagPos.y - 120, 8, GOLD);
        float wave = sinf(time * 8.0f) * 5.0f; // Flapping flag math using Sine waves
    ...
    for (int y = 0; y < MAP_HEIGHT; y++) {
        for (int x = 0; x < MAP_WIDTH; x++) {
            ...
            else if (t == '?') { // Mystery Block
                float pulse = (sinf(time * 5.0f) + 1.0f) * 0.5f; // Glowing animation math
                DrawRectangle(px, py, TILE_SIZE, TILE_SIZE, (Color){200 + pulse*55, 150 + pulse*50, 0, 255});
                DrawRectangleLines(px, py, TILE_SIZE, TILE_SIZE, BLACK);
                DrawText("?", px + 12, py + 6, 28, WHITE);
            }
            else if (t == '^') { // Spikes
                Color spikeCol = (currentTheme == THEME_CASTLE) ? ORANGE : DARKGRAY; // Lava spikes in castle
                DrawTriangle((Vector2){px+20, py+5}, (Vector2){px+5, py+40}, (Vector2){px+35, py+40}, spikeCol);
            }
            else if (t == 'W' || t == 'V' || t == 'U') { // Secret Warp Pipes
                DrawRectangle(px, py, TILE_SIZE, TILE_SIZE, LIME);
                DrawRectangle(px-4, py, TILE_SIZE+8, 10, GREEN); // Pipe rim
                DrawRectangleLines(px-4, py, TILE_SIZE+8, 10, DARKGREEN);
            }
            else if (t == 'C') { // Coins
                float spinWidth = fabs(sinf(time * 3.0f + x)) * 14.0f; // 3D spinning animation
    ...
    for (int i = 0; i < MAX_ENEMIES; i++) {
        ...
        if (enemies[i].type == ENEMY_WADDLER) {
            ...
            DrawRectangle(ex + 6, ey + 6 + fabs(waddle), 4, 8, WHITE); // Eyes
    ...
    // 4. DRAW THE PLAYER (Procedural Sprite Generation)
    if (!player.isDead) {
        float px = player.pos.x; 
        
        // VISUAL OFFSET: The physical hitbox is 30px tall, but the visual sprite is drawn 40px tall. 
        // We subtract 10.0f from the Y so his feet perfectly touch the floor graphically!
        float py = player.pos.y - 10.0f; 
        
        // Dynamic arm/leg swinging based on actual velocity
        float walkCycle = (fabs(player.vel.x) > 10.0f && player.isGrounded) ? sinf(time * 20.0f) * 8.0f : 0.0f;
        if (!player.isGrounded) walkCycle = -5.0f; // Static jumping pose

        // Determine colors based on active player (P1 = Red Quevedo, P2 = Green Luigi)
        ...
        // Face rendering depends on looking Left vs Right
        if (player.facingRight) {
            ...
            DrawRectangle(px + 12, py - 8, 20, 4, pHat); // Hat Brim
        } else {
    ...
    EndMode2D(); // End drawing relative to the camera

    // 6. DRAW UI HUD (Drawn over top of everything, not affected by camera)
...
int main(void) {
    ...
    // Configure tiny, nimble physical hitbox for fairness
    player.width = 25; 
    player.height = 30;
    ...
    while (!WindowShouldClose()) {
        float dt = GetFrameTime(); // Delta Time ensures physics run the same speed on any monitor
        ...
        switch (state) {
            ...
            case STATE_READY: // "Player X Get Ready!" Screen
                if (currentMusic != NULL) UpdateMusicStream(*currentMusic);
                // Synchronize global variables to the current player's personal saved stats
            ...
            case STATE_PLAYING:
                ...
                // --- SECRET WARP PIPE ENTRY LOGIC ---
                // Only works if player pushes Down explicitly while standing on top of a pipe!
                if ((IsKeyDown(KEY_DOWN) || IsKeyDown(KEY_S)) && player.isGrounded) {
                    ...
                    if (pX >= 0 && pX < MAP_WIDTH && pY >= 0 && pY < MAP_HEIGHT) {
                        ...
                        if (t == 'W' || t == 'V' || t == 'U') {
                            PlaySound(sfxClear); 
                            player.score += 5000;
                            // Remember the current level so we can return after finishing the secret area!
                ...
                // --- PLAYER MOVEMENT (Momentum Input) ---
                ...
                // Apply sliding friction if no input is pressed
                ...
                // Generate skid particles if turning around while running fast
                ...
                targetCamY = SmoothCam(targetCamY, desiredY, 0.1f);
                camera.target.x = player.pos.x;
                camera.target.y = targetCamY;
                
                // Clamp camera from panning off the map bounds
            ...
            case STATE_DEAD:
                ...
                if (IsKeyPressed(KEY_ENTER)) {
                    ...
                    // Multiplayer swap logic! Hand off to Player 2 if they have lives left
            ...
            case STATE_VICTORY:
                if (currentMusic != NULL) UpdateMusicStream(*currentMusic);
                if (IsKeyPressed(KEY_ENTER)) {
                    // Check if Player 2 still needs to finish their campaign!
